...
JarlActor.removefromFaction(kmyQuest.CWs.GetPlayerAllegianceEnemyFaction(true))
HousecarlActor.removefromFaction(kmyQuest.CWs.GetPlayerAllegianceEnemyFaction(true))
...
if ((self as Quest) as cwfortsiegemissionscript).SpecialNonFortSiege == 0 && ((self as Quest) as cwfortsiegemissionscript).SpecialCapitalResolutionFortSiege == 0
	...
	if FortLocation != kmyQuest.CWs.FortAmolLocation && FortLocation != kmyQuest.CWs.FortHraggstadLocation
		...
		kmyQuest.CWs.registerMissionSuccess(Alias_Hold.GetLocation(), true)	;if isFortBattle then we won't display the Objective for the hold again, because we've just won the campain
		...
		kmyQuest.CWs.registerMissionSuccess(Alias_Hold.GetLocation(), false)	;for purposes of this function, it's not a fort battle
...
;schofida - The code below is probably not called due to the comment above. Some CWO code is very old which predates some of the Bethesda official patches :(
;			Leaving it anyway because I am a coward.
...
if kmyQuest.CWs.IsPlayerAttacking(Alias_Fort.GetLocation())
	kmyQuest.CWs.CWODefendingActive.value = 1 as Float ;CWO
	self.SetObjectiveFailed(100, true)
	self.setStage(9200) ;FAILURE!
	CWOFailure = true ;CWO
else
	kmyQuest.CWs.CWODefendingActive.value = 0 as Float ;CWO
	self.SetObjectiveCompleted(200, true)
	self.setStage(9000) ;SUCCESS!
	CWOFailure = false ; CWO
endIf
;END CODE
EndFunction
;END FRAGMENT
...
;CWO Start
if kmyQuest.CWs.CWOMission05Key.SendStoryEventAndWait(Alias_Hold.GetLocation(), (kmyQuest.CWs.GetRikkeOrGalmar(-1) as objectreference), none, 0, 0)
	game.GetPlayer().addToFaction(kmyQuest.CWOMission05Faction)
endIf
Alias_FieldCO.ForceRefTo(kmyQuest.CWs.GetRikkeOrGalmar(-1) as objectreference)
if kmyQuest.CWOFortList.hasform(Alias_Fort.GetLocation() as form)
	
else
	kmyQuest.CWs.CWOStartCapital(Alias_Hold.GetLocation())
	self.stop()
endIf
if kmyQuest.CWODoneBeenSiegedYo.hasform(Alias_Fort.GetLocation() as form)
	if kmyQuest.CWs.GetHoldOwner(kmyQuest.CWs.GetHoldID(Alias_Hold.GetLocation())) == kmyQuest.CWs.Playerallegiance
	...
	kmyQuest.CWODoneBeenSiegedYo.addform(Alias_Fort.GetLocation() as form)
endIf
;CWO End
...
kmyQuest.RegisterSpawnDefenderAliasesWithCWReinforcementScript(Alias_RespawnDefenderPhase1A, Alias_RespawnDefenderPhase1B, Alias_RespawnDefenderPhase1C, Alias_RespawnDefenderPhase1D, Alias_RespawnDefenderPhase1FailSafe)
;CWO Start
;if ((self as Quest) as cwfortsiegemissionscript).SpecialNonFortSiege == 0 || ((self as Quest) as cwfortsiegemissionscript).SpecialCapitalResolutionFortSiege == 0 ; schofida edit
if kmyQuest.CWs.IsPlayerAttacking(Alias_Fort.GetLocation())
	kmyQuest.CWs.CWOMurderMayhemScoreBoard(true, 2)
else
	kmyQuest.CWs.CWOMurderMayhemScoreBoard(false, 2)
endIf
...
;else ; schofida edit
;	if kmyQuest.CWs.IsPlayerAttacking(Alias_Fort.GetLocation()) ; schofida edit
;		kmyQuest.CWs.CWOMurderMayhemScoreBoard(true, 2) ; schofida edit
;	else ; schofida edit
;		kmyQuest.CWs.CWOMurderMayhemScoreBoard(false, 2) ; schofida edit
;	endIf ; schofida edit
;	kmyQuest.SetPoolAttackerOnCWReinforcementScript(kmyQuest.CWs.CWOAttackerReinforcements.GetValueInt(), 1.00000, 1.00000, false) ; schofida edit
;	kmyQuest.SetPoolDefenderOnCWReinforcementScript(kmyQuest.CWs.CWODefenderReinforcements.GetValueInt(), 1.00000, 1.00000, false) ; schofida edit
;endIf ; schofida edit
;CWO End
...
if ((self as Quest) as cwfortsiegemissionscript).SpecialNonFortSiege == 1
	...
	kmyQuest.CWs.StartCWCitizensFlee(Alias_Fort.GetLocation())
	self.setStage(10)
elseIf ((self as Quest) as cwfortsiegemissionscript).SpecialCapitalResolutionFortSiege == 1 ; CWO Uncomment
	cwscript.Log("CWFortSiege", "Stage 0: Calling FlagFieldCOWithPotentialMissionFactions() ", 0, false, false) ; CWO Uncomment
	((self as Quest) as cwfortsiegemissionscript).FlagFieldCOWithPotentialMissionFactions(50, false, 1) ;50 = Final Battle at Minor Capital (this quest type) ; CWO Uncomment
endIf
;CWO Start
if kmyQuest.CWs.IsPlayerAttacking(Alias_Fort.GetLocation())
...
if ((self as Quest) as cwfortsiegemissionscript).SpecialNonFortSiege == 0
	...
	if kmyQuest.IsPlayerAttacking()
		...
		if kmyQuest.IsPlayerIsNearAFriendly(false)	;the player hit the trigger near his allies
...
elseIf ((self as Quest) as cwfortsiegemissionscript).SpecialNonFortSiege == 1 || ((self as Quest) as cwfortsiegemissionscript).SpecialCapitalResolutionFortSiege == 1
	((self as Quest) as cwfortsiegemissionscript).FlagFieldCOWithMissionResultFaction(50, true)
endIf
self.stop()
;END CODE
endFunction
...
;If ((self as quest) as CWFortSiegeMissionScript).SpecialCapitalResolutionFortSiege == 1 - CWO Comment out
;; 	CWScript.Log("CWFortSiege", "Stage 1000: setting stage 950 the Jarl Surrenders ") - CWO Comment out
;	
;	if  kmyquest.IsPlayerAttacking() - CWO Comment out
;		setObjectiveCompleted(100) - CWO Comment out
;	endif - CWO Comment out
;
;	setStage(950) - CWO Comment out
;
